...
# Optional: neuer TOC-Parser (nicht erforderlich)
try:
...
def process_pdf(
    ...
    # optional structured toc2 (best effort)
    toc2: Any = {}
    if use_new_toc and parse_protocol_new is not None:
        try:
            tb = parse_protocol_new(flat, capture_offsets=False, debug=False)
            if isinstance(tb, dict):
                toc2 = tb.get("toc") or tb.get("items") or tb
            else:
                toc2 = tb or {}
        except Exception:
            toc2 = {}

    # speeches (these are the "segments" you said you need)
    ...
    # Light inline annotations (only if explicitly requested). Small, safe metadata only.
    if include_light_annotations:
        # normalized speaker names per speech (if available)
        for i, s in enumerate(speeches):
...
def parse_args():
    ...
    ap.add_argument("--no-new-toc", action="store_true")
