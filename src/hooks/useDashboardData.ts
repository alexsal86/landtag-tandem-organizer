...
const fetchSpecialDays = async (tenantId: string | undefined): Promise<SpecialDay[]> => {
  try {
...
const fetchDashboardRpc = async (userId: string, userEmail: string | undefined, tenantId: string): Promise<DashboardRpcData> => {
  try {
    ...
    // Process appointments: filter today/tomorrow
    const now = new Date();
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const tomorrowDate = new Date(today); tomorrowDate.setDate(tomorrowDate.getDate() + 1);

    // Fetch external events (not in RPC since they use a different schema pattern)
...
export const useDashboardData = (): DashboardData => {
  ...
  const { data: rpcData, isLoading } = useQuery({
    ...
    staleTime: 2 * 60_000, // 2 minutes — dashboard data is relatively stable
    gcTime: 5 * 60_000,
    refetchOnWindowFocus: false,
  });
