      ...
      if (upcomingLeaves && upcomingLeaves.length > 0) {
        for (const leave of upcomingLeaves) {
          ...
          if (templates && templates.length > 0) {
            const responseMap = new Map(responses?.map(r => [r.checklist_item_id, r.completed]) || []);
            const unchecked = templates.filter(t => !responseMap.get(t.id));

            if (unchecked.length > 0) {
              toast.warning(
                `Dein Urlaub beginnt morgen! ${unchecked.length} Checklisten-Punkt(e) noch offen: ${unchecked.map(u => u.label).join(", ")}`,
                { duration: 10000 }
              );
            }
          }
      ...
      if (endedLeaves && endedLeaves.length > 0) {
        ...
        if (reminderTemplates && reminderTemplates.length > 0) {
          for (const leave of endedLeaves) {
            ...
            const completedMap = new Map(responses?.map(r => [r.checklist_item_id, r.completed]) || []);
            const toReverse = reminderTemplates.filter(t => completedMap.get(t.id));

            if (toReverse.length > 0) {
              toast.info(
                `Willkommen zurück! Bitte rückgängig machen: ${toReverse.map(t => t.label).join(", ")}`,
                { duration: 15000 }
              );
            }
          }
