...
export function useMyWorkJourFixeSystemData(userId?: string, tenantId?: string) {
  ...
  const loadMeetingSystemData = useCallback(async ({ meetingId, items, meetingDate }: LoadSystemDataParams) => {
          ...
          .filter(
            (decision) =>
              ...
              participantRows.some(
                (participant) => participant.decision_id === decision.id && participant.user_id === userId,
              ),
          )
          .map((decision) => ({
            id: decision.id,
    ...
    try {
      ...
      if (!profiles || profiles.length === 0 || !isMountedRef.current) return;
