import type { ProtocolAgendaItem, ProtocolSpeakerRef } from '@/types/protocol';
import { hasOwnProperty, isRecord } from '@/utils/typeSafety';
import { isProtocolAgendaItem } from '@/utils/protocolGuards';
...
export function isJsonProtocol(value: unknown): value is JSONProtocolStructure {
  ...
  if (hasOwnProperty(value, 'agenda') && value.agenda !== undefined && value.agenda !== null) {
    return Array.isArray(value.agenda) && value.agenda.every(isProtocolAgendaItem);
  }
...
export function parseJSONProtocol(jsonData: unknown): ParsedJSONProtocol {
  ...
  const metadata = {
    ...
    statistics:
      jsonData.statistics || jsonData.stats
        ? {
            ...
            parties_represented:
              jsonData.statistics?.parties_represented ||
              Array.from(new Set(jsonData.speeches.map((s) => s.party).filter((party): party is string => Boolean(party)))),
  ...
  const sessions: ParsedJSONProtocol['structured_data']['sessions'] = [
    {
...
export function getJSONProtocolPreview(jsonData: unknown): {
  ...
  const parties = Array.from(new Set(jsonData.speeches.map((s) => s.party).filter((p): p is string => Boolean(p))));
  const sessionInfo = `Sitzung ${jsonData.session.number || 'unbekannt'} (${jsonData.session.legislative_period || '17'}. Wahlperiode)`;
  const dateInfo = jsonData.session.date ? new Date(jsonData.session.date).toLocaleDateString('de-DE') : 'Datum unbekannt';
