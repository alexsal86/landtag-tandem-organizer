...
import type { ProtocolAgendaItem, ProtocolAnalysisSpeech, ProtocolSessionEvent } from '@/types/protocol';
import { debugConsole } from '@/utils/debugConsole';
import { isPdfTextItem } from '@/utils/protocolGuards';
...
export async function parsePDFFile(file: unknown): Promise<ParsedProtocol> {
  ...
  try {
    ...
    return {
      text: fullText,
      pages,
      metadata: extractMetadata(file.name, fullText, pdf.numPages),
    };
  } catch (error: unknown) {
    debugConsole.error('PDF parsing error:', error);
    throw new Error(`Fehler beim Parsen der PDF: ${error instanceof Error ? error.message : String(error)}`);
  }
}

function detectAgendaItems(text: string): boolean {
  return [/\d+\.\s+[A-ZÄÖÜ]/, /Tagesordnung/i, /TOP\s+\d+/i, /Punkt\s+\d+/i].some((pattern) => pattern.test(text));
}
...
function extractTimeMarkers(text: string): string[] {
  ...
  while ((match = timePattern.exec(text)) !== null) {
    ...
    if (!matches.includes(time)) matches.push(time);
  }
...
function extractMetadata(filename: string, text: string, pageCount: number): ProtocolMetadata {
  ...
  if (filenameMatch) {
    ...
    if (sessionMatch) sessionNumber = sessionMatch[1];
    ...
    if (legislatureMatch) legislature = legislatureMatch[1];
  }
...
export function analyzeProtocolStructure(text: unknown): {
  agendaItems: ReadonlyArray<ProtocolAgendaItem>;
  ...
  const lines = smartLineSplit(preprocessProtocolText(text));
  const agendaItems: ProtocolAgendaItem[] = [];
  ...
  for (const line of lines) {
    const agendaMatch = patterns.agendaItem.map((pattern) => line.match(pattern)).find(Boolean);
    if (agendaMatch) {
      agendaItems.push({
        number: agendaMatch[1],
        agenda_number: agendaMatch[1],
        title: agendaMatch[2].replace(/^-\s*/, '').trim(),
        item_type: determineItemType(agendaMatch[2]),
      });
      continue;
    }

    const timeMatch = Array.from(line.matchAll(patterns.time))[0];
    if (timeMatch) {
      lastTime = `${timeMatch[1].padStart(2, '0')}:${timeMatch[2]}`;
      const sessionMatch = Array.from(line.matchAll(patterns.sessionEvent))[0];
      if (sessionMatch) {
        const eventType = sessionMatch[1].toLowerCase();
        let sessionType: ProtocolSessionEvent['session_type'] = 'start';
        if (eventType.includes('ende') || eventType.includes('schluss')) sessionType = 'end';
        ...
        sessions.push({ session_type: sessionType, timestamp: lastTime, notes: line.trim() });
      }
      continue;
    }

    const speakerMatch = patterns.speaker.map((pattern) => line.match(pattern)).find(Boolean);
    if (speakerMatch) {
      ...
      currentSpeaker = speakerMatch[2] ? `${speakerMatch[1].trim()} (${speakerMatch[2].trim()})` : speakerMatch[1].trim();
    ...
    if (interjectionMatch) {
      ...
      let speechType: ProtocolAnalysisSpeech['speech_type'] = 'interjection';
      if (patterns.applause.test(content)) speechType = 'applause';
      else if (patterns.objection.test(content)) speechType = 'interruption';
      speeches.push({ speaker_name: 'Parlament', speech_content: content, speech_type: speechType });
      continue;
    }
...
function smartLineSplit(text: string): string[] {
  ...
  for (const line of lines) {
    if (line.length > 500) {
      const sentences = line.split(/(?<=[.!?])\s+(?=[A-ZÄÖÜ])/).filter((segment) => segment.trim().length > 0);
      processedLines.push(...(sentences.length > 1 ? sentences : [line]));
    } else {
