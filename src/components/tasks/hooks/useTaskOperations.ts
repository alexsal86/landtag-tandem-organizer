...
export function useTaskOperations({
  ...
  const handleSubtaskComplete = async (subtaskId: string, isCompleted: boolean, result: string = '') => {
    try {
      const subtask = assignedSubtasks.find((s: any) => s.id === subtaskId);
      if (!subtask) return;
      ...
      if (subtask.source_type === 'call_followup' && isCompleted && subtask.call_log_id) {
        ...
        const updateData: any = { is_completed: isCompleted, completed_at: isCompleted ? new Date().toISOString() : null };
  ...
  const handleCallFollowUpComplete = async (callLogId: string, resultText?: string) => {
    const { data: callLog, error: callLogError } = await supabase.from('call_logs').select('*, contact_id, caller_phone, caller_name').eq('id', callLogId).single();
  ...
  const createQuickNoteFromTask = async (taskId: string, content: string) => {
  ...
  return {
    processingTaskIds, showCelebration, setShowCelebration,
    toggleTaskStatus, addComment, snoozeTask, snoozeSubtask,
    updateSnooze, deleteSnooze, handleSubtaskComplete,
    createQuickNoteFromTask, completeTodo,
  };
}
