...
export function ContactImport() {
            ...
            const parsedData = vcards.map((vcard: any) => {
              ...
              if (vcard.n) {
                ...
                if (Array.isArray(name)) {
                  ...
                  if (name[3])
                    data["Titel"] =
                      (data["Titel"] ? data["Titel"] + " " : "") + name[3]; // Prefix
                }
              }
              ...
              if (vcard.email) {
                const emails = Array.isArray(vcard.email)
                  ? vcard.email
                  : [vcard.email];
                emails.forEach((email: any, index: number) => {
                  const emailValue =
                    typeof email === "object" ? email.valueOf() : email;
              ...
              if (vcard.tel) {
                const phones = Array.isArray(vcard.tel)
                  ? vcard.tel
                  : [vcard.tel];
                let businessCount = 0;
                let privateCount = 0;
                ...
                phones.forEach((phone: any) => {
                  const phoneValue =
                    typeof phone === "object" ? phone.valueOf() : phone;
                  const phoneType = phone?.type || [];
                  const types = Array.isArray(phoneType)
                    ? phoneType
                    : [phoneType];
                  ...
                  if (
                    ...
                    types.some((t: string) => t?.toLowerCase().includes("home"))
                  ) {
                    if (privateCount === 0) {
              ...
              // Addresses
              if (vcard.adr) {
                const addresses = Array.isArray(vcard.adr)
                  ? vcard.adr
                  : [vcard.adr];
                ...
                addresses.forEach((address: any) => {
                  ...
                  if (Array.isArray(adrValue) && adrValue.length >= 7) {
                    ...
                    if (
                      ...
                      if (postalCode)
                        data["Geschäftlich: Postleitzahl"] = postalCode;
  ...
  const autoMapFields = (sourceFields: string[]) => {
    const mappings: FieldMapping[] = sourceFields.map((sourceField) => ({
      sourceField,
      targetField:
        FIELD_MAPPINGS[sourceField as keyof typeof FIELD_MAPPINGS] || "",
    }));
  ...
  const importContact = async (rowIndex: number) => {
    ...
    try {
      ...
      if (error) {
        setErrors((prev) => [
          ...prev,
          `Zeile ${rowIndex + 1}: ${error.message}`,
        ]);
      } else {
      ...
      setErrors((prev) => [
        ...prev,
        `Zeile ${rowIndex + 1}: Unbekannter Fehler`,
      ]);
    }
  };
  ...
  const processRow = async (rowIndex: number) => {
    ...
    if (!contactData.name || contactData.name.trim() === "") {
      setErrors((prev) => [
        ...prev,
        `Zeile ${rowIndex + 1}: Kein Name angegeben`,
      ]);
      continueImport();
      return;
    }
    ...
    if (contactData.email && !isValidEmail(contactData.email)) {
      setErrors((prev) => [
        ...prev,
        `Zeile ${rowIndex + 1}: Ungültige E-Mail-Adresse (${contactData.email})`,
      ]);
      continueImport();
      return;
    }
  ...
  const finishImport = () => {
    const message =
      `${importedCount} Kontakte erfolgreich importiert` +
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-2">
                Unterstützte Formate: CSV, Excel (XLSX, XLS), OpenDocument
                (ODS), vCard (VCF)
              </p>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Ordnen Sie die Spalten aus Ihrer Datei den entsprechenden Feldern
              zu:
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                {data.length} Kontakte werden importiert. Überprüfen Sie die
                ersten Einträge:
              </p>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="ask">
                      Bei jedem Duplikat nachfragen
                    </SelectItem>
                    <SelectItem value="skip">
                      Alle Duplikate automatisch überspringen
                    </SelectItem>
                    <SelectItem value="import">
                      Alle Duplikate trotzdem importieren
                    </SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground mt-2">
                  Legt fest, wie mit möglichen Duplikaten umgegangen werden
                  soll.
                </p>
              </div>
            </div>
                ...
                <AlertDescription>
                  <div>
                    <p className="font-medium mb-2">
                      {errors.length} Fehler beim Import:
                    </p>
                ...
                <AlertDescription>
                  <div>
                    <p className="font-medium mb-2 text-yellow-800">
                      {duplicateWarnings.length} mögliche Duplikate gefunden:
                    </p>
                    <div className="max-h-32 overflow-auto">
                      ...
                      {duplicateWarnings.length > 5 && (
                        <p className="text-sm text-yellow-700">
                          ... und {duplicateWarnings.length - 5} weitere
                        </p>
                      )}
                    </div>
                  </div>
