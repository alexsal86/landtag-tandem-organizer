...
export function MatrixLoginForm() {
  ...
  const deriveHomeserverUrl = (userId: string): string => {
    // Extract server from @user:server.tld -> https://server.tld
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      await supabase
        .from('profiles')
        .update({
          matrix_user_id: confirmedUserId,
          matrix_access_token: newAccessToken,
          matrix_homeserver_url: effectiveHomeserver,
        })
        .eq('user_id', user.id)
        .eq('tenant_id', currentTenant.id);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="matrix-password">Passwort</Label>
            <Input
              id="matrix-password"
              type="password"
              placeholder="Ihr Matrix-Passwort"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && !isLoggingIn) handlePasswordLogin();
              }}
            />
            <p className="text-xs text-muted-foreground">
              Wird nur für die Anmeldung verwendet und nicht gespeichert
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Wird automatisch aus der User ID abgeleitet, kann aber manuell überschrieben werden
            </p>
          </div>

          <Button onClick={handlePasswordLogin} disabled={isLoggingIn || !matrixUserId.trim() || !password.trim()} className="w-full">
            {isLoggingIn ? (
        ...
        <Collapsible open={showAdvanced} onOpenChange={setShowAdvanced}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" size="sm" className="w-full justify-between text-muted-foreground">
              Erweitert: Manueller Access Token
        ...
        {isConnected && (
          <div className="space-y-2 rounded-md border p-3 bg-muted/30">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Startet eine Verifizierungsanfrage an Ihren eigenen Matrix-Account (SAS/Emoji-Verifizierung).
            </p>
            <div className="flex flex-wrap gap-2">
