...
export function MyWorkCasesWorkspace() {
  ...
  const handleQuickVisibilityChange = async (item: CaseItem, nextPublic: boolean) => {
    const ok = await applyItemOptimisticUpdate(
      item.id,
      (row) => ({ ...row, is_public: nextPublic }),
      () => supabase.from("case_items").update({ is_public: nextPublic } as any).eq("id", item.id),
      "Sichtbarkeit konnte nicht geändert werden.",
    );
    if (ok) toast.success(nextPublic ? "Vorgang ist jetzt öffentlich sichtbar." : "Vorgang ist jetzt intern sichtbar.");
  };
  ...
  const handleCaseItemSave = async () => {
    ...
    const patch = {
      ...
      is_public: editableCaseItem.isPublic,
                                      ...
                                      >
                                        <ContextMenu>
                                            ...
                                            >
                                              <div className="hidden xl:grid h-12 grid-cols-[28px_40px_minmax(140px,1fr)_minmax(200px,2fr)_80px_80px_minmax(90px,0.8fr)_minmax(100px,0.8fr)_50px_100px] items-center gap-2 text-xs text-muted-foreground">
                                                ...
                                                <span className="truncate text-sm font-medium text-foreground inline-flex items-center gap-1">
                                                  {getItemSubject(item)}
                                                  {item.is_public && <Globe className="h-3.5 w-3.5 text-emerald-600 shrink-0" />}
                                                  {/* 3c: Link indicator */}
                                                  {linkedFile && (
                                          ...
                                          <ContextMenuContent className="w-56">
                                            ...
                                            <ContextMenuItem onClick={() => runAsync(() => handleQuickVisibilityChange(item, !item.is_public))}>
                                              <Globe className="mr-2 h-3 w-3" />
                                              {item.is_public ? "Sichtbarkeit: Intern" : "Sichtbarkeit: Öffentlich"}
                                            </ContextMenuItem>
