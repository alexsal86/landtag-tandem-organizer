                ...
                {props.sortedCaseItems.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    <div className="hidden xl:grid grid-cols-[28px_34px_minmax(220px,1.8fr)_minmax(380px,3.6fr)_72px_minmax(80px,0.7fr)_minmax(90px,0.7fr)_36px_44px_92px] gap-2 border-b px-2 pb-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                      ...
                      <span className="group inline-flex items-center gap-0.5">Kategorie<button type="button" className={props.sortButtonClass("category", "asc")} onClick={() => props.toggleSort("category", "asc")}><ArrowUp className="h-3 w-3" /></button><button type="button" className={props.sortButtonClass("category", "desc")} onClick={() => props.toggleSort("category", "desc")}><ArrowDown className="h-3 w-3" /></button></span>
                    ...
                    {props.sortedCaseItems.map((item, index) => {
                      ...
                      const category = props.getCategory(item);
                                ...
                                <ContextMenuTrigger asChild>
                                  <button type="button" className={cn("w-full px-2 py-2 text-left transition-colors hover:bg-muted/40", isActive && "bg-primary/5", props.focusedItemIndex >= 0 && props.focusedItemIndex === index && "ring-1 ring-primary/40")} onClick={() => props.handleSelectCaseItem(item)}>
                                    ...
                                    <div className="space-y-3 xl:hidden">
                                      <div className="flex items-start justify-between gap-3"><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-muted text-muted-foreground"><ChannelIcon className="h-3 w-3" /></span><p className="truncate text-sm font-medium flex-1">{props.getItemSubject(item)}</p>{item.visible_to_all && <Globe className="h-3.5 w-3.5 text-blue-500 shrink-0" />}{linkedFile && <Link2 className="h-3.5 w-3.5 text-blue-500 shrink-0" />}</div>{props.getItemDescription(item) && <p className="text-xs text-muted-foreground line-clamp-2 mb-2">{props.getItemDescription(item)}</p>}<div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">{category ? <Badge variant="outline" className="text-[10px] px-1.5 py-0">{category}</Badge> : <Badge variant="outline" className="text-[10px] px-1.5 py-0 border-amber-500/40 text-amber-600">Kategorie fehlt</Badge>}<Badge variant="outline" className={cn("text-[10px] px-1.5 py-0", props.getStatusMeta(item.status).badgeClass)}>{props.getStatusMeta(item.status).label}</Badge><span className="inline-flex items-center gap-1" title={props.priorityMeta(item.priority).label}><Circle className={cn("h-2.5 w-2.5 fill-current", props.priorityMeta(item.priority).color)} /></span>{contactDisplay && <span className="text-[10px]">👤 {contactDisplay}</span>}</div></div><span {...dragProvided.dragHandleProps} className="inline-flex items-center justify-center cursor-grab text-muted-foreground/50 hover:text-muted-foreground pt-1" onClick={(e) => e.stopPropagation()}><GripVertical className="h-4 w-4" /></span></div>
