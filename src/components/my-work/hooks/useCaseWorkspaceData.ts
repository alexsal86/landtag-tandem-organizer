...
export const useCaseWorkspaceData = ({ tenantId, userId }: { tenantId?: string; userId?: string }) => {
  ...
  const fetchItemsPage = useCallback(async (offset: number) => {
    ...
    const { data, error } = await supabase
      .from("case_items")
      .select("id, is_public, subject, summary, resolution_summary, source_channel, source_received_at, status, completion_note, completed_at, priority, due_at, case_file_id, user_id, owner_user_id, intake_payload, updated_at")
      .eq("tenant_id", tenantId)
