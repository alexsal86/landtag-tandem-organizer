...
export function useCreateAppointment(open: boolean, onOpenChange: (open: boolean) => void) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setAppointmentLocations((locs.data || []).map((location: { id: string; name: string; address?: string | null }) => ({ ...location, address: location.address ?? undefined } as AppointmentLocationOption)));
