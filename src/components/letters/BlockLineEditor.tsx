...
const LINE_FONT_OPTIONS: Array<'Calibri' | 'Cambria'> = ['Calibri', 'Cambria'];
const resolveLineFontFamily = (fontFamily?: 'Calibri' | 'Cambria') => fontFamily || 'Calibri';
                      ...
                      >
                        ...
                        {line.type === 'spacer' ? (
                          ...
                          <div className="flex flex-1 items-center gap-1">
                            ...
                            <Select
                              value={resolveLineFontFamily(line.fontFamily)}
                              onValueChange={(value: 'Calibri' | 'Cambria') => updateLine(line.id, { fontFamily: value })}
                            >
              ...
              return columns.map((col, ci) => (
                <div key={ci} style={{ width: col.widthUnit === 'cm' ? `${col.widthValue}cm` : `${col.widthValue}%` }}>
                  {col.items.map((line) => {
                    ...
                    return (
                      <div key={line.id} style={{ fontSize: `${line.fontSize || 8}pt`, fontFamily: resolveLineFontFamily(line.fontFamily), fontWeight: line.valueBold ? 'bold' : 'normal', color: line.color || '#000' }}>
        ...
        <div className="space-y-0 text-[9pt] leading-tight">
          {lines.map((line) => {
            ...
            if (line.type === 'text-only') {
              return (
                <div key={line.id} className={`flex items-center gap-1 ${line.valueBold ? 'font-bold' : ''}`} style={{ fontSize: `${line.fontSize || 9}pt`, fontFamily: resolveLineFontFamily(line.fontFamily), color: line.color || undefined }}>
            ...
            // label-value
            return (
              <div key={line.id} className="flex items-center gap-1" style={{ fontSize: `${line.fontSize || 9}pt`, fontFamily: resolveLineFontFamily(line.fontFamily), color: line.color || undefined }}>
