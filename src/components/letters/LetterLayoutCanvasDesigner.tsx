...
type BlockKey = 'addressField' | 'infoBlock' | 'subject' | 'content' | 'footer' | 'attachments' | 'header';
...
export function LetterLayoutCanvasDesigner({ layoutSettings, onLayoutChange, onJumpToTab }: Props) {
  ...
  const getRect = (key: BlockKey): Rect => {
    switch (key) {
      case 'header': return { x: 0, y: 0, w: localLayout.pageWidth, h: localLayout.header.height };
  ...
  const updateByRect = (key: BlockKey, rect: Rect) => {
    setLocalLayout(prev => {
      ...
      if (key === 'header') {
        next.header.height = clamp(snapMm(rect.h), 20, 70);
  ...
  const moveSelectedByKey = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (!dx && !dy) return;
    e.preventDefault();
    if (disabledBlocks.has(selected)) return;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Doppelklick auf einen Block öffnet den passenden Tab. Pfeiltasten verschieben den ausgewählten Block um 1mm.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[320px_1fr] gap-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {BLOCKS.map((block) => (
                <div key={block.key} className="flex items-center justify-between gap-2">
                  <Button type="button" variant={selected === block.key ? 'default' : 'outline'} size="sm" className="flex-1" onClick={() => setSelected(block.key)}>{block.label}</Button>
        ...
        <div tabIndex={0} onKeyDown={moveSelectedByKey} className="border rounded-lg p-4 bg-muted/20 overflow-auto outline-none" onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp}>
          <div className="mx-auto bg-white shadow-xl relative select-none" style={{ width: pagePx.w, height: pagePx.h }}>
            {showRuler && (
              <>
                <div className="absolute top-0 left-0 right-0 h-5 border-b bg-white/90 text-[9px] text-muted-foreground pointer-events-none">
                  {Array.from({ length: Math.floor(localLayout.pageWidth / 10) + 1 }).map((_, i) => (
                    <span key={`rx-${i}`} className="absolute" style={{ left: i * 10 * SCALE }}>{i * 10}</span>
                  ))}
                </div>
                <div className="absolute top-0 bottom-0 left-0 w-5 border-r bg-white/90 text-[9px] text-muted-foreground pointer-events-none">
                  {Array.from({ length: Math.floor(localLayout.pageHeight / 10) + 1 }).map((_, i) => (
                    <span key={`ry-${i}`} className="absolute" style={{ top: i * 10 * SCALE }}>{i * 10}</span>
                  ))}
                </div>
              </>
            )}
