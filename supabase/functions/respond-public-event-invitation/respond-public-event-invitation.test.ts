...
function createMockClient(overrides?: {
  ...
  updates?: Array<{ table: string; values: Record<string, unknown>; eq: [string, string] }>;
}) {
        ...
        select() {
          return {
            eq(_column: string, value: string) {
              return {
                maybeSingle: async () => {
                  if (table === 'event_rsvp_public_links') return { data: overrides?.publicLink ?? null, error: null };
                  if (table === 'event_rsvps') return { data: overrides?.rsvp ?? null, error: null };
                  if (table === 'event_plannings') return { data: overrides?.event ?? null, error: null };
                  throw new Error(`Unexpected select table ${table} for ${value}`);
                },
              };
            },
          };
...
describe('respond-public-event-invitation', () => {
  it('rejects malformed public write requests', async () => {
    const response = await handleRespondPublicEventInvitation(
      new Request('http://localhost/functions/v1/respond-public-event-invitation', {
        method: 'POST',
  ...
  it('rejects inactive invitations and overlong comments', async () => {
    const expiredResponse = await handleRespondPublicEventInvitation(
      new Request('http://localhost/functions/v1/respond-public-event-invitation', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ public_code: 'a'.repeat(32), status: 'accepted' }),
      }),
      {
        createServiceRoleClient: () => createMockClient({
          publicLink: {
    ...
    await expect(commentResponse.json()).resolves.toMatchObject({ code: 'comment_too_long' });
  });

  it('updates RSVP and public link metadata for a valid response', async () => {
    const updates: Array<{ table: string; values: Record<string, unknown>; eq: [string, string] }> = [];
    ...
    const response = await handleRespondPublicEventInvitation(
      new Request('http://localhost/functions/v1/respond-public-event-invitation', {
        ...
        body: JSON.stringify({ public_code: 'c'.repeat(32), status: 'tentative', comment: ' Komme etwas später ' }),
      }),
      {
        createServiceRoleClient: () => createMockClient({
          publicLink: {
    ...
    for (let index = 0; index < RATE_LIMIT_MAX_REQUESTS + 1; index += 1) {
      latest = await handleRespondPublicEventInvitation(
        new Request('http://localhost/functions/v1/respond-public-event-invitation', {
          ...
          body: JSON.stringify({ public_code: 'd'.repeat(32), status: 'accepted' }),
        }),
        {
