-- Create letter management system
...
-- Insert default letter template for all existing tenants
...
SELECT 
  ...
  (SELECT utm.user_id FROM public.user_tenant_memberships utm 
   WHERE utm.tenant_id = t.id AND utm.role IN ('abgeordneter', 'bueroleitung') 
   ORDER BY utm.created_at ASC LIMIT 1) as created_by,
  true as is_default
