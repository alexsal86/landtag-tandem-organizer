-- Create letter management system (fixed)
...
-- Insert default letter template for tenants that have users (fixed version)
INSERT INTO public.letter_templates (tenant_id, name, letterhead_html, letterhead_css, created_by, is_default)
SELECT 
  t.id as tenant_id,
  'Standard Briefkopf' as name,
  ...
  </div>' as letterhead_html,
  '.letterhead { margin-bottom: 2rem; }
   .letterhead h1 { font-size: 1.5rem; font-weight: bold; margin-bottom: 1rem; }
   .address, .contact { margin-bottom: 0.5rem; }' as letterhead_css,
  utm.user_id as created_by,
  true as is_default
FROM public.tenants t
INNER JOIN public.user_tenant_memberships utm ON utm.tenant_id = t.id 
WHERE t.is_active = true 
  AND utm.role IN ('abgeordneter', 'bueroleitung')
  AND utm.is_active = true
GROUP BY t.id, utm.user_id
HAVING utm.created_at = (
  SELECT MIN(utm2.created_at) 
  FROM public.user_tenant_memberships utm2 
  WHERE utm2.tenant_id = t.id 
    AND utm2.role IN ('abgeordneter', 'bueroleitung')
