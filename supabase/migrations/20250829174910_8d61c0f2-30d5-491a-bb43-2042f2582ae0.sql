-- Fix task notifications to send to correct users
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF NEW.assigned_to IS NOT NULL AND NEW.assigned_to != '' THEN
      ...
      FOREACH assigned_user_id IN ARRAY assigned_user_ids
      LOOP
        ...
        IF assigned_user_id != NEW.user_id::text AND assigned_user_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
          PERFORM public.create_notification(
            assigned_user_id::uuid,
            'task_assigned',
            'Aufgabe zugewiesen',
      ...
      IF NEW.assigned_to IS NOT NULL AND NEW.assigned_to != '' THEN
        ...
        LOOP
          ...
          IF assigned_user_id != NEW.user_id::text AND assigned_user_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
            ...
            IF OLD.assigned_to IS NULL OR OLD.assigned_to = '' OR position(assigned_user_id IN OLD.assigned_to) = 0 THEN
              PERFORM public.create_notification(
                assigned_user_id::uuid,
                'task_assigned',
                'Aufgabe zugewiesen',
