...
-- Ensure we only have the correct triggers
-- These should be the only triggers for notifications:
-- handle_knowledge_document_notifications_trigger (already exists)
-- task_notifications_trigger (already exists) 
-- handle_message_notifications_trigger (already exists)
-- handle_message_recipient_notifications_trigger (already exists)

-- Add unique constraint to prevent duplicate notifications
ALTER TABLE notifications ADD CONSTRAINT IF NOT EXISTS notifications_unique_per_user_type_data 
UNIQUE (user_id, notification_type_id, data);
