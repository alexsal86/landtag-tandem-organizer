...
| get-public-event-invitation       |      false | public-readonly    | Lädt RSVP-Gastdaten ausschließlich per `public_code`, validiert Widerruf/Ablauf, liefert Ablauf-/Policy-Metadaten zurück und nutzt gestaffelte IP-Rate-Limits für normale sowie ungültige Lookup-Versuche |
| respond-public-event-invitation   |      false | public-webhook     | Schreibt RSVP-Antworten ausschließlich per starkem `public_code`, akzeptiert vorbereitete CAPTCHA-Felder, dokumentiert `latest_wins` als Antwortlogik und protokolliert Missbrauchsmetadaten strukturiert |
